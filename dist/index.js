...
function sha2563(value, to_) {
  const to = to_ || "hex";
  const bytes = sha256(isHex(value, { strict: false }) ? toBytes3(value) : value);
  if (to === "bytes")
    return bytes;
  return toHex(bytes);
}
...
// src/providers/client.ts
function instantiateClient(account, rpcUrl) {
...
var ClientProvider = class {
  ...
  constructor(runtime) {
    ...
    this.client = instantiateClient(this.account, rpcUrl ?? void 0);
  }
};
var clientProvider = {
...
async function getParamsWithLLM(runtime, message, template, state = null, maxAttempts = 5) {
...
var readContractAction = {
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    ...
    const result = await clientProvider2.client.readContract({
...
// src/actions/writeContract.ts
import {
  elizaLogger
} from "@elizaos/core";
var writeContractTemplate = `
...
var writeContractAction = {
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    ...
    const result = await clientProvider2.client.writeContract({
      address: options.contractAddress,
      functionName: options.functionName,
      args: options.functionArgs,
      value: options.value,
...
// src/actions/deployContract.ts
import {
  elizaLogger as elizaLogger2
} from "@elizaos/core";
import fs from "fs";
var deployContractTemplate = `
...
var deployContractAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.info("Starting deploy contract action");
    elizaLogger2.debug("User message:", message.content.text);
    const clientProvider2 = new ClientProvider(runtime);
    const options = await getParamsWithLLM(
      runtime,
      message,
...
var getTransactionAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger3.info("Starting get transaction action");
    elizaLogger3.debug("User message:", message.content.text);
    const clientProvider2 = new ClientProvider(runtime);
    const options = await getParamsWithLLM(
      runtime,
      message,
    ...
    if (!options || !options.hash || !/^0x[a-fA-F0-9]{64}$/.test(options.hash)) {
      elizaLogger3.error("No valid transaction hash found in message");
      throw new Error("No valid transaction hash found in message");
    }
    elizaLogger3.info(
      `Getting transaction details for hash: ${options.hash}`
    );
    const result = await clientProvider2.client.getTransaction({
      hash: options.hash
    });
    elizaLogger3.success("Successfully retrieved transaction details");
    elizaLogger3.debug("Transaction details:", result);
    await callback(
      {
...
// src/actions/getCurrentNonce.ts
import {
  elizaLogger as elizaLogger4
} from "@elizaos/core";
var getCurrentNonceAction = {
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    elizaLogger4.info("Starting get current nonce action");
    elizaLogger4.debug("User message:", message.content.text);
    const clientProvider2 = new ClientProvider(runtime);
    const addressMatch = message.content.text.match(/0x[a-fA-F0-9]{40}/);
    if (!addressMatch) {
      elizaLogger4.error("No valid address found in message");
      throw new Error("No valid address found in message");
    }
    elizaLogger4.info(
      `Getting current nonce for address: ${addressMatch[0]}`
    );
    const result = await clientProvider2.client.getCurrentNonce({
      address: addressMatch[0]
    });
    elizaLogger4.success(`Successfully retrieved nonce: ${result}`);
    await callback(
      {
...
// src/actions/waitForTransactionReceipt.ts
import {
  elizaLogger as elizaLogger5
} from "@elizaos/core";
var waitForTransactionReceiptAction = {
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    elizaLogger5.info("Starting wait for transaction receipt action");
    elizaLogger5.debug("User message:", message.content.text);
    const clientProvider2 = new ClientProvider(runtime);
    const hashMatch = message.content.text.match(/0x[a-fA-F0-9]{64}/);
    if (!hashMatch) {
      elizaLogger5.error("No valid transaction hash found in message");
      throw new Error("No valid transaction hash found in message");
    }
    elizaLogger5.info(`Waiting for receipt of transaction: ${hashMatch[0]}`);
    const result = await clientProvider2.client.waitForTransactionReceipt({
      hash: hashMatch[0]
    });
    elizaLogger5.success("Successfully received transaction receipt");
    elizaLogger5.debug("Transaction receipt:", result);
    await callback(
      {
...
// src/actions/getContractSchema.ts
import {
  elizaLogger as elizaLogger6
} from "@elizaos/core";
var getContractSchemaAction = {
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    elizaLogger6.info("Starting get contract schema action");
    elizaLogger6.debug("User message:", message.content.text);
    const clientProvider2 = new ClientProvider(runtime);
    const addressMatch = message.content.text.match(/0x[a-fA-F0-9]{40}/);
    if (!addressMatch) {
      elizaLogger6.error("No valid address found in message");
      throw new Error("No valid address found in message");
    }
    elizaLogger6.info(
      `Getting contract schema for address: ${addressMatch[0]}`
    );
    const result = await clientProvider2.client.getContractSchema(
      addressMatch[0]
    );
    elizaLogger6.success("Successfully retrieved contract schema");
    elizaLogger6.debug("Contract schema:", result);
    await callback(
      {
...
var index_default = genLayerPlugin;
export {
